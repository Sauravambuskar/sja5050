...
import type { ExtractedNomineeData } from '@/lib/ocr';
...
interface NomineeFormProps {
  nomineeOcrFill?: ExtractedNomineeData | null;
  onNomineeOcrFillApplied?: () => void;
}

export const NomineeForm = ({ nomineeOcrFill, onNomineeOcrFillApplied }: NomineeFormProps) => {
  ...
  useEffect(() => {
    if (!nomineeOcrFill) return;
    setEditingNominee(null);
    form.reset({
      full_name: nomineeOcrFill.full_name || '',
      relationship: '',
      dob: nomineeOcrFill.dob ? new Date(nomineeOcrFill.dob) : null,
      blood_group: nomineeOcrFill.blood_group || '',
    });
    setIsDialogOpen(true);
    toast.info('Nominee details pre-filled from document. Please select relationship and save.');
    onNomineeOcrFillApplied?.();
  }, [nomineeOcrFill]);
  ...
  const onSubmit = (values: NomineeFormValues) => mutation.mutate(values);
                    ...
                    <TableRow key={nominee.id}>
                      ...
                      <TableCell className="text-right">
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This will permanently delete the nominee.</AlertDialogDescription></AlertDialogHeader>
                            <AlertDialogFooter>
