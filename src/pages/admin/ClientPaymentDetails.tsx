...
const fetchInvestorPayments = async (userId: string) => {
  const { data, error } = await supabase.rpc('get_client_investor_payment_details', { p_user_id: userId });
  if (error) throw error;
  return data as InvestorPayment[];
};

const fetchIntroducerPayments = async (userId: string) => {
  const { data, error } = await supabase.rpc('get_client_introducer_payment_details', { p_user_id: userId });
  if (error) throw error;
...
const ClientPaymentDetails = () => {
  ...
  const { data: investorPayments, isLoading: investorLoading } = useQuery({
    queryKey: ['investorPayments', userId],
    queryFn: () => fetchInvestorPayments(userId!),
    enabled: !!userId,
  });

  const { data: introducerPayments, isLoading: introducerLoading } = useQuery({
    queryKey: ['introducerPayments', userId],
    queryFn: () => fetchIntroducerPayments(userId!),
    enabled: !!userId,
  });
  ...
  return (
    <>
      <Button asChild variant="outline" className="mb-4">
        <Link to="/admin/users"><ArrowLeft className="mr-2 h-4 w-4" /> Back to User Management</Link>
      </Button>
