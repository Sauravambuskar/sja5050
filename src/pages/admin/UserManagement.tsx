...
const UserManagement = () => {
  ...
  const renderDesktopView = () => {
    ...
    if (isLoading && !users) {
      ...
      tableBodyContent = (
        <TableRow>
          <TableCell colSpan={6} className="text-center text-red-500">Error fetching users: {error.message}</TableCell>
        </TableRow>
      );
    } else {
      tableBodyContent = (
        <>
          {users?.map((user) => (
            <TableRow key={user.id} className={isUserSuspended(user) ? "bg-muted/50" : ""}>
              ...
              <TableCell>
                {isUserSuspended(user) ? (<Badge variant="destructive">Suspended</Badge>) : (<Badge variant={user.role === 'admin' ? 'destructive' : 'outline'}>{user.role}</Badge>)}
              </TableCell>
              <TableCell>
                <Badge variant={user.kyc_status === "Approved" ? "default" : user.kyc_status === "Pending Review" ? "outline" : "secondary"}>{user.kyc_status || 'Not Submitted'}</Badge>
              </TableCell>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={() => navigate(`/admin/users/${user.id}/payment-details`)}>
                      <FileText className="mr-2 h-4 w-4" />Payment Details
                    </DropdownMenuItem>
      ...
      <AlertDialog open={isSuspendDialogOpen} onOpenChange={setIsSuspendDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This action will {isUserSuspended(userToSuspend) ? 'reinstate' : 'suspend'} the user account for '{userToSuspend?.full_name}'. They will {isUserSuspended(userToSuspend) ? 'be able to' : 'not be able to'} log in.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
