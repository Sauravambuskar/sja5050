...
serve(async (req) => {
  ...
  try {
    ...
    await supabaseAdmin.from('admin_audit_log').insert({
        ...
        details: { reason: 'Admin login as user from User Management page.' }
    })

    // 5. Return the magic link URL
    const magicLink = `${Deno.env.get('SUPABASE_URL')}/auth/v1/verify?token=${data.properties.hashed_token}&type=magiclink&redirect_to=/`

    return new Response(JSON.stringify({ magicLink }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
