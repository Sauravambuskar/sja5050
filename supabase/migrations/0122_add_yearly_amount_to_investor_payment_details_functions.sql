-- Migration to add yearly_amount to investor payment details functions

-- Drop existing functions to recreate them with the new column
DROP FUNCTION IF EXISTS public.get_my_investor_payment_details();
DROP FUNCTION IF EXISTS public.get_client_investor_payment_details(p_user_id uuid);

-- Recreate get_my_investor_payment_details with yearly_amount
CREATE OR REPLACE FUNCTION public.get_my_investor_payment_details()
 RETURNS TABLE(invt_id text, invt_date date, invt_amount numeric, rate numeric, per_day_amount numeric, total_days integer, total_interest numeric, month_amount numeric, yearly_amount numeric)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
    SELECT
        ...
        (ui.investment_amount * (ip.annual_rate / 100.0))::numeric(10, 2) as yearly_amount
    FROM
...
-- Recreate get_client_investor_payment_details with yearly_amount
