-- 1) User receipt function (SECURITY DEFINER to safely read payout_log)
CREATE OR REPLACE FUNCTION public.get_my_payout_receipt(p_investment_id uuid, p_payout_month date)
RETURNS TABLE(
  user_id uuid,
  user_name text,
  user_email text,
...
-- 2) Admin receipt function (admin-only)
CREATE OR REPLACE FUNCTION public.get_payout_receipt_for_admin(p_investment_id uuid, p_payout_month date)
RETURNS TABLE(
  user_id uuid,
  user_name text,
  user_email text,
...
-- 3) Enhance record_payout to create notifications with receipt links when marked Paid
CREATE OR REPLACE FUNCTION public.record_payout(p_investment_id uuid, p_payout_month date, p_status text, p_paid_amount numeric, p_payment_mode text, p_remarks text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- When Paid: create notifications with links to receipts
  IF p_status = 'Paid' THEN
    -- Fetch details for message/links
    ...
    -- User notification with client receipt link
    ...
    -- Admin (actor) notification with admin receipt link
    INSERT INTO public.notifications (user_id, title, description, type, link_to)
    VALUES (
      auth.uid(),
