-- Drop the existing functions to modify their parameters
DROP FUNCTION IF EXISTS public.get_all_deposit_requests(text, integer, integer);
DROP FUNCTION IF EXISTS public.get_all_deposit_requests_count(text);

-- Recreate the function to fetch deposit requests with a new search parameter
...
RETURNS TABLE(
    request_id uuid, user_name text, user_id uuid, user_email text, 
    amount numeric, reference_id text, requested_at timestamp with time zone, 
    status text, screenshot_path text, admin_notes text, wallet_balance numeric
)
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Recreate the count function with the new search parameter
