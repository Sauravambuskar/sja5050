-- Function to get admin KYC overview
CREATE OR REPLACE FUNCTION public.get_admin_kyc_overview(
...
BEGIN
  ...
  WITH user_docs AS (
    ...
    GROUP BY
      p.id, u.email, p.full_name, p.aadhaar_number, p.pan_number, p.kyc_status
  )
  SELECT
    ud.user_id,
    ud.full_name,
...
-- Function to count KYC overview results
CREATE OR REPLACE FUNCTION public.get_admin_kyc_overview_count(
  p_search_text TEXT DEFAULT NULL,
  p_status_filter TEXT DEFAULT NULL
)
RETURNS BIGINT
...
-- Function to process user KYC (approve/reject)
CREATE OR REPLACE FUNCTION public.admin_process_user_kyc(
  p_user_id UUID,
  p_new_status TEXT,
  p_admin_notes TEXT
)
