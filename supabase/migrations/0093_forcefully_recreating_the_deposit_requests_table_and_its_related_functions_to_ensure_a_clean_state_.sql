-- Forcefully drop dependent functions first to break any locks
...
-- Recreate the admin dashboard stats function now that all dependencies are present
CREATE OR REPLACE FUNCTION public.get_admin_dashboard_stats()
 RETURNS TABLE(total_users bigint, aum numeric, pending_kyc bigint, pending_withdrawals_count bigint, pending_withdrawals_value numeric, pending_deposits_count bigint, pending_deposits_value numeric, pending_investments_count bigint, pending_investments_value numeric, pending_investment_withdrawals_count bigint, pending_investment_withdrawals_value numeric, monthly_payout_projection numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
    ...
    SELECT
        ...
        (SELECT COUNT(*) FROM public.investment_withdrawal_requests WHERE status = 'Pending') as pending_investment_withdrawals_count,
        (SELECT COALESCE(SUM(ui.investment_amount), 0) FROM public.investment_withdrawal_requests iwr JOIN public.user_investments ui ON iwr.investment_id = ui.id WHERE iwr.status = 'Pending') as pending_investment_withdrawals_value,
...
-- Recreate the admin activity feed function
