...
-- Drop old deposit functions (if they exist with specific signatures)
...
-- Recreate the deposit_requests table
...
CREATE POLICY "Admins can manage all deposit requests" ON public.deposit_requests FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

-- Recreate deposit functions
...
BEGIN
    ...
    SELECT
        ...
        wr.details || jsonb_build_object(
            ...
            'plan_name', CASE WHEN wr.type = 'Investment' AND wr.details->>'investment_id' IS NOT NULL THEN (SELECT name FROM investment_plans ip WHERE ip.id = (wr.details->>'investment_id')::uuid) ELSE NULL END,
            'investment_amount', CASE WHEN wr.type = 'Investment' AND wr.details->>'investment_id' IS NOT NULL THEN (SELECT ui.investment_amount FROM user_investments ui WHERE ui.id = (wr.details->>'investment_id')::uuid) ELSE NULL END
        )
